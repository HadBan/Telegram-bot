...
# =====================================================
OWNER_IDS = [6276293498, 5019808756]  # Второй ID замените на свой
# =====================================================
...
def save_insult_to_library(word: str) -> bool:
    """Сохраняет новое оскорбление в библиотеку"""
    global insults_library
...
def remove_insult_from_library(word: str) -> bool:
    """Удаляет оскорбление из библиотеки"""
    global insults_library
    word = word.lower().strip()
...
def contains_insult(text):
    """Проверяет текст на оскорбления из базового списка и библиотеки"""
...
async def check_insult_with_groq(text, chat_id):
    """Проверяет оскорбления через AI и самообучается"""
...
async def cmd_learned_insults(update, context):
    """Показывает список выученных оскорблений"""
    if insults_library:
...
async def cmd_add_insult(update, context):
    """Добавляет оскорбление в библиотеку (только владелец)"""
...
async def cmd_remove_insult(update, context):
    """Удаляет оскорбление из библиотеки (только владелец)"""
    if not await require_owner(update, context):
        return
    if not context.args:
...
async def cmd_clear_insults(update, context):
    """Очищает библиотеку выученных оскорблений (только владелец)"""
    if not await require_owner(update, context):
        return
    global insults_library
